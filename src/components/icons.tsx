...
export const PageLoader: React.FC<{ message?: string, color?:`blue`|`green` }> = ({ message, color="blue" }) => (
  <div className="fixed inset-0 flex flex-col items-center justify-center gap-4 bg-white/60 backdrop-blur-sm z-40">
    <div className="relative w-16 h-16">
      <div className={`absolute inset-0 rounded-full border-4 ${color === "green" ? "border-green-500/30" : "border-blue-500/30"}`}></div>
      <div className={`absolute inset-0 rounded-full border-4 ${color === "green" ? "border-green-500" : "border-blue-500"} border-t-transparent animate-spin`}></div>
    </div>
